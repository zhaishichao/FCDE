...
from deap.tools import selRandom, selTournamentDCD, selNSGA2
...
def selTournament(individuals, k, tournsize=7):
    chosen = []
    for i in range(k):
        chose = []
        # 找到距离大于等于0且角度最大的个体
...
class DSSMOTE_P:
    ...
    def evaluate(self, individuals):
        ...
        for j, individual in enumerate(individuals):
            if not individual.fitness.valid:
                ...
                # 计算角度
                a = np.linalg.norm(self.maj_center - self.min_center)
                b = maj_dis
                c = min_dis
                if b == 0 or c == 0:
                    angle = 0
                ...
                individual.fitness.values = (angle, maj_min_dis)
    ...
    def init_toolbox(self):
        ...
        creator.create("Individual", gp.PrimitiveTree, fitness=creator.FitnessMulti, feasible=None)
