...
for ax, metric_name, big_vals, small_vals in [
    ...
    if SHOW_STAT_LEGEND:
        ...
        stat_order = list(zip(CATEGORY_COLORS, [f"{n}" for n in counts]))
        ax.legend(
            handles=[legend_handle(c, STAT_MARKER_SIZE) for c, _ in stat_order],
            labels=[l for _, l in stat_order],
            loc="upper left",
            fontsize=STAT_LEGEND_SIZE,
            title=STAT_LEGEND_TITLE,
