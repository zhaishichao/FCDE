...
# =========================
title = 'D17'
dataset_name = 'new-thyroid_' + title.lower() + '_dg'
root_path = "C:\\Users\\zsc\\Desktop\\FCDE实验结果汇总\\tsne\\"
file_name = "new-thyroid1_5_X_train_res_dg.csv"
...
plt.legend(
    # title="Class",  # 图例标题
    fontsize=12,  # 👈 控制图例文字大小（主标签）
    loc="upper right",
    title_fontsize=11,  # 👈 标题字体大小（略大一点更协调）
    markerscale=0.8,  # 👈 缩小图例中点的大小（默认1.0）
    handletextpad=0.15,  # 👈 点与文字之间的距离（关键！默认约0.8）
    labelspacing=0.2,  # 👈 每一行之间的间距（默认较大）
    borderpad=0.25,  # 👈 图例内容与边框的距离
