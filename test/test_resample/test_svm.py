...
if not os.path.exists(save_path_smoten):
    os.makedirs(save_path_smoten)
...
if __name__ == '__main__':
    ...
    df_mean_smoten = pd.DataFrame(columns=columns_datasets)
    ...
    for index, datasetname in enumerate(datasetnames):
        ...
        df_smoten = pd.DataFrame(columns=columns_dataset)
        ...
        for i in range(num_run):
            clf = SVC(kernel='linear', C=1.0, probability=True, random_state=42 + i)
        ...
        df_mean_smoten.loc[index] = [datasetname, num_instances, num_features, df_smoten['F-measure'].mean(),
                                     df_smoten['G-mean'].mean(), df_smoten['AUC'].mean()]
        ...
        df_smoten.to_csv(save_path_smoten + datasetname + '.csv', encoding='utf_8_sig', index=False)
        ...
        df_mean_smoten.to_csv(save_path + 'mean_smoten.csv', encoding='utf_8_sig', index=False)
